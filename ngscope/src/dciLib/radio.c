...
int radio_init_and_start(srsran_rf_t* rf, 
    ...
    do {
      ...
      if (ret < 0) {
        ERROR("Error searching for cell, mib was not decoded or cell was not found");
        exit(-1);
    ...
    printf("number of PRBs of the cell: %d \n", cell->nof_prb);
    int sampling_rate = srsran_sampling_freq_hz(cell->nof_prb);
    if (sampling_rate != -1) {
      printf("Setting sampling rate %.2f MHz\n", (float)sampling_rate / 1e6);
      float sampling_rate_rf = srsran_rf_set_rx_srate(rf, (double)sampling_rate);
      printf("Actual sampling rate :%.2f MHz\n",sampling_rate_rf / 1e6);
      if (sampling_rate_rf != sampling_rate) {
