...
// initializing plot data for status_plot.c
...
void wait_for_radio(ngscope_status_tracker_t* q, int nof_dev){
    ...
    while(true){
        ...
        pthread_mutex_unlock(&cell_mutex);
        //get only cells that have PRBs > 0, which should be just one cell
        if(radio_ready){
            break;
        }else{
            usleep(10000);
    ...
    printf("finishgin waiting for the radio \n");
    return;
}
...
// add data for plotting in status_plot
int status_tracker_handle_plot(ngscope_status_buffer_t* dci_buffer){
    ...
    // printf("enqueueing cell downlink PRB data");
    ...
    //    printf("enqueueing cell uplink PRB data");
...
void* status_tracker_thread(void* p){
    ...
    if(dis_plot == 0){
        ...
        // plotting function called inside status_plot.c!!!
