          ...
          if (decode_pdsch) {
            ...
            for (uint32_t tm = 0; tm < 4 && !n; tm++) {
              ...
              if ((ue_dl_cfg.cfg.tm == SRSRAN_TM1 && cell.nof_ports == 1) ||
                ...
                n = srsran_ue_decode_dci_yx(&ue_dl, &dl_sf, &ue_dl_cfg, &pdsch_cfg, &dci_res);

                if(dci_res.nof_ul_dci > 0){
                    ...
                    if (srsran_ra_ul_dci_to_grant(&ue_dl.cell, &ul_sf, &ul_hopping, dci_res.dci_ul, &ul_grant)) {
                         continue; 
                    } 
                    phich_set_pending_ack(&ack_list, TTI_RX_ACK(tti), ul_grant.n_prb_tilde[0], dci_res.dci_ul[0].n_dmrs); 
                    printf("FOUND UL DCI: tti:%d hop:%d dmrs:%d\n", tti, dci_res.dci_ul[0].freq_hop_fl, dci_res.dci_ul[0].n_dmrs);
                    break;
                }
                if(dci_res.nof_dl_dci > 0){
                    //printf("FOUND DL DCI: tti:%d rnti:%d format:%d \n", tti, dci_res.dci_dl[0].rnti, dci_res.dci_dl[0].format);
                }
              }
            }
            if(pdsch_cfg.rnti > 0){
                ...
                ack_available = decode_phich(&ue_dl, &dl_sf, &ue_dl_cfg, &ack_list, &phich_res);

                if(dci_res.nof_ul_dci > 0){
                    fprintf(fp, "%d\t%d\t%d\t%d\t%d\t", tti, pdsch_cfg.rnti, ul_grant.L_prb, ul_grant.tb.mcs_idx, ul_grant.tb.rv);
                }else{
                    fprintf(fp, "%d\t%d\t%d\t%d\t%d\t", tti, 0, 0, 0, 0);
                }
